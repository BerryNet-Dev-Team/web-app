...
export const useAuthStore = defineStore('auth', {
  ...
  getters: {
    ...
    // Check if user has access to dataset management
    datasetPermissionGranted(state) {
      const allowedRoles = [
        "ADMIN",
        "DATASET_MANAGER",
        "AI_TECHNICIAN"
      ...
      return allowedRoles.findIndex(r => r === state.role) != -1;
    },

    // Check if user has access to the IA inferences
    inferencesPermissionGranted(state) {
      const allowedRoles = [
        "ADMIN",
        "AI_USER",
        "AI_TECHNICIAN"
